'use strict';

const express = require('express');
const router = express.Router();
var createError = require('http-errors');

const Agente = require('../../models/Agente');

/**
 * GET /
 * Retorna una lista de agentes
 */

router.get('/', async(req, res, next) => {
    try {
        const agentes = await Agente.find().exec();
        //!Si await fall, lanza una excepción ---> throw new Exception()
        res.json({ succes: true, result: agentes });
    } catch (err) {
        next(err);
    }
});
/**
 * GET /: id
 * Retorna un agente
 */
router.get('/:id', async(req, res, next) => {
    try {
        const _id = req.params.id;

        const agente = await Agente.findById(_id).exec();
        if (!agente) {
            next(createError(404));
            return;
        }
        res.json({ succes: true, result: agente });
    } catch (err) {
        next(err);
    }
})


/**
 * POST /
 * Crea una agente en la colección
 */
router.post('/', async(req, res, next) => {
    try {
        const datosAgente = req.body;

        //Crear un agente en memorioa
        const agente = new Agente(datosAgente);

        //Guardarlo en la base de datos
        const agenteGuardado = await agente.save();

        res.json({ succes: true, result: agenteGuardado });
    } catch (err) {
        next(err);
    }
});

/**
 * DELETE/ :id
 * Elimina un agente
 */
router.delete('/:id', async(req, res, next) => {
    try {
        const _id = req.params.id;

        await Agente.remove({ _id: _id }).exec();

        res.json({ succes: true });
    } catch (err) {
        next(err);
    }
});

/**
 * PUT /:id
 * Actuaiza un agente
 */
router.put('/:id', async(req, res, next) => {
    try {
        const _id = req.params.id;
        const datosAgente = req.body;
        //Usamos {new:true} para que devuelva el agente actualizado
        const agenteActualizado = await Agente.findByIdAndUpdate(_id, datosAgente, { new: true }).exec();
        res.json({ succes: true, result: agenteActualizado });
    } catch (err) {
        next(err);
    }
})

module.exports = router;